...
def fourier(price,period,method='difference'):
    ...
    for i in range(0,len(period)):
        coeffs =[]
        for j in range(period[i],len(price)-period[i]):
        ...
        df = pd.DataFrame(coeffs,index=price.iloc[period[i]:-period[i]].index)
...
def wadl(price,period):
    ...
    for i in range(0,len(period)):
        ...
        WAD = WAD.cumsum()
        WAD = pd.DataFrame(WAD,index=price.iloc[period[i]:-period[i]].index)
        WAD.columns = ['close']
        dict[period[i]] = WAD
...
def Market (prices,periods):
    ...
    for i in range(0,len(periods)):
        ms=[]
        for j in range(periods[i],len(prices)-periods[i]):
            if prices.open[j+1] < prices.close[j+1]:
                ms.append(1)
            elif prices.open[j+1] >= prices.close[j+1]:
                ms.append(0)
